...
const ALIAS_PATTERN = /^[a-zA-ZÀ-ÿ0-9_ -]+$/

const db = getDatabase()

export function validateRegistering(requestBody: any)
{
	let {login, password, passwordValidation, alias } = requestBody
	login = sanitizeInput(login)
	alias = sanitizeInput(alias)
	password = sanitizeInput(password)
	passwordValidation = sanitizeInput(passwordValidation)
	...
	requestBody.passwordValidation = passwordValidation
}
...
{
	let { login, password } = requestBody
	login = sanitizeInput(login)
	password = sanitizeInput(password)
	console.log(`[login] : ${login}\n[password] : ${password}`)
	if (!login)
